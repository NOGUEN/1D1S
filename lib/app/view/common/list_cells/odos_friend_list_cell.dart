...
class ODOSFriendListCell extends StatelessWidget {
  ...
  final FriendsRequestCode friendsRequestCode;
  const ODOSFriendListCell(
      {super.key, required this.friendName, required this.friendsRequestCode});

  @override
  Widget build(BuildContext context) {
    return BaseODOSFriendListCell(widgetList: [
...
class BaseODOSFriendListCell extends StatelessWidget {
  ...
  const BaseODOSFriendListCell({super.key, required this.widgetList});
...
Widget friendRequestButton() {
  return Container(
    ...
    height: 30.h,
    decoration: BoxDecoration(
        color: AppColors.gray100, borderRadius: BorderRadius.circular(8.w)),
    child: TextButton(
