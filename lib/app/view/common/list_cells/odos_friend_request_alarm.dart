...
class ODOSFriendRequestAlarm extends StatelessWidget {
  ...
  const ODOSFriendRequestAlarm({super.key, required this.friendName});
...
class BaseODOSFriendRequestAlarm extends StatelessWidget {
  ...
  const BaseODOSFriendRequestAlarm({super.key, required this.widgetList});
...
Widget friendRequestMessage(String friendName) {
  return Row(
    children: [
      ...
      Flexible(
        child: Text('$friendName님이 친구 요청을 보냈어요.', style: listCellTextStyle),
      ),
    ],
  );
}
